...
sources:
  - id: vllm-agentx
    ...
    title: "vLLM on the AgentX benchmark: agentic serving (source of blog.vllm.ai)"
...
**Prefer session affinity over load balancing.** Queue-depth, running-token and KV-utilization routing
policies were all reported to underperform simple session-sticky routing, because moving a session off
its warm GPU forces a KV fetch whose prefetched blocks then occupy KV capacity at the
destination.[^vllm-agentx] **The load-balancing win is real but it is smaller than the cache-locality
loss it causes**, whenever the inter-turn delay is short enough that the prefix is still resident. This
is the opposite of the standard stateless-service default, and it is the most directly actionable
finding here.

**Do not make pipeline parallelism the default for warm agentic turns.** Because a warm turn
contributes only a few hundred fresh tokens, pipeline bubbles consume the gain; the reported guidance
is to keep pipeline parallelism for cold, compute-heavy prefills rather than applying it
uniformly.[^vllm-agentx] Chunked pipeline parallelism is reported as not rescuing this.[^vllm-agentx]

**Do not budget decode-context-parallelism gains by analogy from another model family.** After
substantial communication/compute overlap work, decode context parallelism only *matched* decode
expert parallelism on DeepSeek V4 — a result that did not transfer from pure-MLA models such as
DeepSeek R1 or Kimi K2.5/K3.[^vllm-agentx] **Architecture, not just scale, decides whether DCP
pays.** Independent corroboration that DCP is not a universally applicable lever arrived in the same
week from the engine itself: `AttentionImplBase.supports_dcp` was flipped from defaulting `True` to
defaulting `False`, requiring each attention backend to opt in explicitly, with eleven in-tree
backends doing so and anything outside that list — including out-of-tree and custom backends — now
failing closed at startup rather than running an untested path.[^vllm-55780]

**Capacity-plan against total tokens, but read the metric's definition first.** The headline figure is
up to 83K total tokens per GPU-second on DeepSeek V4 Pro at P90 interactivity ≥ 50 tokens/s/user,
measured on 12× GB300 at concurrency 256.[^vllm-agentx] **That metric counts input, output *and cached*
tokens**, which inflates it substantially relative to any output-token-only measure — a >96% cache hit
rate means most of what is counted was never recomputed.[^vllm-agentx] It is a legitimate measure of
work served; it is not comparable to an output-token throughput number, and the two should never be
placed side by side.
...
- The post is vendor-authored (vLLM Team and Inferact, work led by Inferact, with NVIDIA and AMD
  support). Its mitigation is that the benchmark itself is third-party — SemiAnalysis AgentX, with
  public traces, a public harness, and per-row links to the hosted run — which is the strongest
  external validation in this week's crop. But the same third-party dashboard underpins the other
  major serving post of the week, so those two results are not fully independent of each
  other.[^vllm-agentx]
...
[^vllm-agentx]: vLLM blog, "vLLM on the AgentX benchmark" (source id: vllm-agentx), read at repo commit `d2fe8cf`, post dated 2026-09-08 (verified from the `_posts/` filename, the frontmatter `date: 2026-09-08 23:00:00 +0000`, and `git log` on the file: added 2026-09-08). The workload-shape statistics, the three "bitter lessons" (pipeline parallelism on warm turns, decode context parallelism not transferring from pure-MLA models, load-balancing policies losing to session-sticky routing and the KV-fetch mechanism behind it), the 83K tokens/GPU-second figure with its 12× GB300 / concurrency 256 / P90 ≥ 50 tok/s conditions, the statement that the metric counts cached tokens, and the serving-cost disclaimer are all stated in the post. Benchmark is SemiAnalysis AgentX (public traces, harness at `SemiAnalysisAI/agentx-harness`); figures are tabulated rather than read off a chart.
